...
format.new = function() {
	...
	var result = {"model":"steve","variant":"java_edition","resolution":128,"texture":"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEAAAABACAQAAAAAYLlVAAAAOUlEQVR42u3OIQEAAAACIP1/2hkWWEBzVgEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAYF3YDicAEE8VTiYAAAAAElFTkSuQmCC","pose":true,"layer_template":false};
...
const skin_dialog = new Dialog({
	...
	onConfirm(result) {
		...
		if (result.model == 'flat_texture') {
			...
			var result = {"model":"steve","variant":"java_edition","resolution":"128","texture":"","pose":true,"layer_template":false};
			if (newProject(format)) {
				let preset = skin_presets[result.model];
				let model = JSON.parse(preset.model || (result.variant == 'java_edition' ? preset.model_java : preset.model_bedrock));
				codec.parse(model, result.resolution/16, result.texture, result.pose, result.layer_template);
