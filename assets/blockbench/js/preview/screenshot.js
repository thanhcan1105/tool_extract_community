...
const Screencam = {
	...
	gif_options_dialog: new Dialog({
		...
		form: {
			format: {
				label: 'dialog.create_gif.format', type: 'select', options: {
			...
			'_1': '_',
			length_mode: {
				label: 'dialog.create_gif.length_mode', type: 'select', default: 'seconds', options: {
	 	...
	 	Canvas.withoutGizmos(function () {
			...
			if (options.crop !== false) {
				...
				if (display_mode && display_slot === 'gui') {
					...
					// Screencam.returnScreenshot(frame.canvas.toDataURL(), cb);
					returnScreenshotDataSkin(frame.canvas.toDataURL(), cb);
				}
			} else {
				...
				if (result === 0 && buttons.length == 3) {
					if (navigator.clipboard && navigator.clipboard.write) {
						fetch(dataUrl).then(async data => {
							...
							let aa = await blobToBase64(blob);
							console.log(aa)
...
async function returnScreenshotDataSkin(dataUrl, cb, blob) {
...
//		console.log("result: " + result);
        ...
        window.parent.postMessage({ type: "image", text: "result: " + result }, "*");
	});
}
...
BARS.defineActions(function () {
	...
	Screencam.timelapse_dialog = new Dialog({
		...
		form: {
			...
			source: {
				label: 'dialog.timelapse.source', type: 'select', value: 'preview', options: {
